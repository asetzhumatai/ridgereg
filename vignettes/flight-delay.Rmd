...
```{r}
# Ensure consistent factor levels across all splits
for (col in names(train)) {
  if (is.factor(train[[col]]) && col %in% names(valid)) {
    valid[[col]] <- factor(valid[[col]], levels = levels(train[[col]]))
  }
...
lambda_grid <- data.frame(lambda = seq(0.1, 10, by = 0.5))  # Adjust as needed



# --- Data preparation before loop ---
train <- train %>% mutate(time_hour_num = as.numeric(time_hour)) %>% select(-time_hour)
valid <- valid %>% mutate(time_hour_num = as.numeric(time_hour)) %>% select(-time_hour)
test  <- test  %>% mutate(time_hour_num  = as.numeric(time_hour)) %>% select(-time_hour)
...
valid_rmse <- sapply(lambda_grid$lambda, function(lam) {
  ...
  preds <- predict(model, newdata = valid[, setdiff(names(valid), "arr_delay")])
